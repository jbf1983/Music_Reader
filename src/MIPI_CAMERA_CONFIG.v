...
//test
  ...
  output      [15:0]  R_DATA,
	...
	output              TR,
	output              I2C_SCL_O,
  output reg          MIPI_CAMERA_RELEASE  
	);
...
2: begin 
   ...
   if ( W_POINTER_END ) begin  W_POINTER_GO  <=0; ST<=3 ; DELAY<=0;  end
	end                // Write pointer
3: begin 
    DELAY  <= DELAY +1;
    if ( DELAY ==2 ) begin 
...
//read DATA 		 
6: begin 
	if ( R_END ) begin  R_GO  <=0; ST<=7 ; DELAY<=0; end
	end                
7: begin 
    DELAY  <=DELAY +1;
    if (DELAY ==2 ) begin 	 
...
31: begin 
      if  ( SLV8_REG16_DATA8[31:24] == DELAY_TYPE  )  ST <= 40 ;     
	      ...
	      if ( W_WORD_END ) begin  W_WORD_GO  <=0; ST<=32 ;  DELAY<=0;  end
		end 	
...
35: begin 
        if  (  WCNT == WORD_NUM_MAX )    //13
		  begin   
		       ST<= ST ; 	 
		       I2C_LO0P <= 1 ;  
				 MIPI_CAMERA_RELEASE <=1 ;  
			  end 
	     else   ST<=31 ; 	 
	 end 
	 
36: begin  //END 
		       WCNT <=0;  CNT <=0; DELAY <= 0; 
	 end 
...
41: begin 
    if ( DELAY == SLV8_REG16_DATA8[7:0] * 1   )   begin 
	  ST<=42; 
	  WCNT <=WCNT+16'd1 ;		
	 end  
	 else  DELAY <=DELAY +1;	
...
assign I2C_SCL   = (ST==0) ? 1'b0 : ((I2C_SCL_O) ? 1'b1 : 1'b0);
//==== I2C WRITE WORD ===
I2C_WRITE_WDATA wrd(
   .RESET_N      ( RESET_N_1),
	.PT_CK        ( CLK_400K),
...
case ( WCNT ) 
     ...
     2	 :SLV8_REG16_DATA8 <= { TIME_DELAY, 16'h0000,   8'd10};
     ...
     7	 :SLV8_REG16_DATA8 <= { TIME_DELAY, 16'h0000,   8'd10};
    ...
    11	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h0304,   8'd03}; // PLL pll1_div_mipi
    ...
    36	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h3500,   4'h0, iREG_EXPO[19:16] }; // exposure HH
    37	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h3501,         iREG_EXPO[15:8] }; // exposure H 2c
    38	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h3502,         iREG_EXPO[7:0] }; // exposure L 00
    39	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h3503,   8'h00}; // gain no delay, exposure no delay
    40	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h3508,   3'h0, iREG_GAIN[12:8] }; // gain H. was 8'h02 0f
    41	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h3509,         iREG_GAIN[7:0] }; // gain L
    42	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h3700,   8'h48}; // sensor control
 ...
 //136	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h37b9,   8'hff}; // sensor control // 800x600
 //137	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h3808,   8'h03}; // X output size H. 0x02
 //138	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h3809,   8'h20}; // X output size L. 0x80
 //139	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h380a,   8'h02}; // Y output size H. 0x01
 //140	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h380b,   8'h58}; // Y output size L. 0xE0 // 60 fps (combined with pll settings)
   ...
   154	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h3830,   8'd08}; // ablc_use_num[5:1]
   155	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h3836,   8'd02}; // zline_use_num[5:1]
...
// The fix for the gray haze was to use the line below to turn ON *manual* BLC offsets. And then entering that offset in register 0x4013.
...
// Turn lens correction LENC[7] off. Not needed for the D8M module.
   ...
   209	 :SLV8_REG16_DATA8 <= { 8'h6c,      16'h5e00,   iREG_TEST_PATTERN}; // test pattern
