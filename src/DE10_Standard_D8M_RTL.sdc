...
#SDRAM CLK
create_generated_clock  -name clk_dram_ext [get_ports {DRAM_CLK}] \
                        -source [get_pins {i_SDRAM_PLL|sdram_pll_inst|altera_pll_i|general[1].gpll~PLL_OUTPUT_COUNTER|divclk}]


#VGA CLK
create_generated_clock  -name clk_vga_ext [get_ports {VGA_CLK}] \
                        -source [get_pins {i_MIPI_PLL|mipi_pll_inst|altera_pll_i|general[1].gpll~PLL_OUTPUT_COUNTER|divclk}] \
                        -invert
...
set_multicycle_path -from [get_clocks {clk_dram_ext}] \
                    -to [get_pins {i_SDRAM_PLL|sdram_pll_inst|altera_pll_i|general[0].gpll~PLL_OUTPUT_COUNTER|divclk}] \
                    -setup 2           
...
# max  0.2+0.1 =  0.3
# min -1.5-0.1 = -1.6
set_output_delay -max -clock clk_vga_ext  0.3 [get_ports {VGA_R* VGA_G* VGA_B* VGA_BLANK_N}]
set_output_delay -min -clock clk_vga_ext -1.6 [get_ports {VGA_R* VGA_G* VGA_B* VGA_BLANK_N}]

#**************************************************************
# Set Clock Groups
#**************************************************************
set_clock_groups -asynchronous -group [get_clocks {i_MIPI_PLL|mipi_pll_inst|altera_pll_i|general[0].gpll~PLL_OUTPUT_COUNTER|divclk}] \
